...
export const Reports = () => {
  ...
  const transactionProduct = async () => {
    try {
      const transactionProduct = await api.get(`/transaction-products`);
  ...
  const countTransactionProduct = async () => {
    try {
      const transactionProduct = await api.get(`/transaction-products/count`);
  ...
  return (
    <Box h={"100vh"} p={"0 25px"}>
      <Flex justifyContent={"space-between"} w={"100%"} gap={"10px"} h={"50%"}>
